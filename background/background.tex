...
Chip Multi-core Processors (CMPs) are now ubiquitous in the desktop, server and smartphone space due to the difficulty in scaling single-core performance.
...
\section{Heterogeneous Chip Multi-core Processors}~\label{sec:hcmp}
...
%In their 2014 paper, Venkat {\it et al.~}\cite{venkat2014harnessingisa} demonstrate that a multi-ISA HCMP can improve performance by up to 1.4x and achieve energy savings of up to 40\% compared to a CMP on a peak-power budget of 40W.
...
\paragraph*{Micro-architectural Reconfigurable Dynamic Multi-core Processors}
...
This section covers the principles of value prediction, which is used in Chapter~\ref{chp:hardchanges} to resolve data-dependencies at runtime.
The different types of predictors are described, and then, the value predictor used in the chapter is explained in detail.
...
%However, recent work by Perais {\it et al.~}\cite{peraisVTAGE2014,peraisBeBop2015} shows that by adopting the prediction and update scheme of the ITTAGE branch predictor~\cite{SeznecITTAGE}, and grouping predictions into blocks, value predictors be efficiently implemented.
...
As predictions are updated at commit time, if multiple blocks with the same PC are live, this can potentially cause multiple value mispredictions as the information in the value predictor may be stale.
