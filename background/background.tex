...
A composition fetches instructions from a single source and executes them accross all the physical cores that compose it.
...
Unlike VLIW that uses static placement and static issue which puts high pressure on the compiler, EDGE allows for dynamic issue which distributes responsability between the hardware and the compiler more evenly.
...
Instead of storing each observed value for an instruction, a \textit{Computational} predictor stores the \textit{stride}, and the last commited value of an instruction.
...
This section covers the different machine learning techniques and evaluation model used throught the thesis.
