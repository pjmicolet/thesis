...
The two main assumption made is that a core can only ever execute a single block at a time, and once the block is committed, it can remain in the instruction window as a temporary buffer.
...
Once the values of these instructions are produced they are then forwarded to the successive speculative block via a register bypass.
Using this techniques, Robatmili et al. show that for a 16 core composition (TFlex processor), this register bypassing technique with criticality detection improves performance on a set of integer spec benchmarks by 1.08x on average and up to 1.16x at best.\
