...
Whilst this may be the case for TFlex, as it can only execute a single block per core, an E2 type processor is designed to not have idle cores in its composition.
This is achieved by allowing cores to fetch multiple blocks in its instruction window and execute them in parallel, thus no block can be in the instruction window purely for storage.
