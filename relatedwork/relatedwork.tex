...
\paragraph*{Summary}
Whilst CoreFusion and Bahurupi use traditional ISAs, they must either support a limited version of composition where a single 8 eight word block is distributed amongst cores (CoreFusion), or add extra instructions which increases code block size by up to 24\% (Bahurupi).
...
This means that new applications will need to be executed multiple times in order to generate a profile which is then used by the DMP to know how to reconfigure itself.
This procedure which can be costly if the number of ways the processor can be reconfigured is high (for example when a programmer must choose between multithreading and core composition).
...
\paragraph*{Summary}
Data prefetching can help alleviate the effect of data-dependencies by ensuring that the instructions fire quicker.
...
Yet it does not allow for instructions to execute with speculative data, which, as will be seen in Chapter~\ref{chp:hardchanges} increases ILP in large core compositions.
...
\section{Summary}
This chapter covered the related work relevant to the research conducted throughout this thesis.
The section underlined that EDGE based processors benefit from the fact that instruction dependencies are determined by the compiler and encoded in the ISA, meaning that fetching does not require a centralised unit for register renaming or instruction numbering.
This makes EDGE a more attractive architecture to use for core composition as it lends itself better to the technique~\cite{kim2007tflex}.
The fact that E2 has a segmented instruction window means that when the program executing features small blocks, it can have more blocks in flight than a TFlex processor given the same number of cores composed, thus allowing for more ILP to be exploited.
This makes E2 the more attractive processor to explore, and it is used as a base in this thesis.

Then the different techniques for automatically configuring a DMP were explained.
Overall, they rely on profiling applications in order to make decisions.
This means that programs will need to be executed multiple times in order to create a profile which is then used to configure the processor later on.
...
This was followed by exploring different hardware techniques used to reduce the energy consumption of applications.
Techniques to determine when to migrate threads to different cores and when to use DVFS were described and whilst they are effective ways of increasing energy efficiency, they rely on the processor cores to be designed ahead of time, and often sacrifice some speed to reduce energy consumption.
...
Whilst thread level speculation can improve the perfromance of some applications, they require considerable architectural modifications and software-level support.

Value prediction and data-prefetching then shown to be two methods of reducing the impact of data-dependencies between instructions.
...
As value prediction allows multiple instructions to execute with speculative data, this is a more promising approach for increasing ILP in large core compositions, as will be explained in Chapter~\ref{chp:hardchanges}.
...
This is to demonstrate that machine learning is becoming a promising method for this domain.
