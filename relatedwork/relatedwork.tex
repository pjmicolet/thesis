...
The Bahurupi~\cite{pricopi2012bahurupi} polymorphic heterogeneous multicore architecture proposed by Pricopi {\it et.~al} introduces a sentinel instruction which informs the hardware about the \textit{live-in} and \textit{live-out} registers of a basic block.
...
In their work~\cite{kim2007tflex} Kim {\it et.~al} show that a 32 core composition can outperforms a single TFlex core by 3x on a set of EEMBC benchmarks and SPEC2000 microbenchmarks.
...
Watanabe {\it et.~al} propose a different type of dynamic multicore processor, where cores share execution units ~\cite{Watanabe2010Widget}.
...
Using this classifier Gupta {\it et.~al} are able to improve performance per watt of a set of single and multithreaded benchmarks by 93\%, 81\% and 6\% compared to an interactive, on demand and powersave governor respectively.
...
Hertzberg {\it et.~al} push the idea of speculative multithreading further in ~\cite{dbtspec2011} by using dynamic binary translation (DBT) to generate optimised parallel code on the fly, they name this the Runtime Automatic Speculative Parallelism technique (RASP).
...
Swarm is extended by Abeydeera {\it et.~al} their speculation aware multithreading policy SAM~\cite{Abeydeera2017SpecMulti}.
...
Farhad {\it et.~al} also use integer linear programming in~\cite{farhad2012streamilp} to schedule StreamIt programs on multicore.
...
The work of Wang {\it et.~al}~\cite{wang2013partitionstreamit} motivated the use of machine learning for streaming applications however it focused on homogeneous multicore processors.
