...
Having a block with many values means that the D-VTAGE predictor can predict a higher number of register reads for a single EDGE block, however this comes at the expense of having less blocks in the predictor tables.
...
To determine the size, all the EDGE blocks of the SD-VBS benchmarks are analysed to determine the average register read and write count is per block.
...
This is normal: lower confidence allows predictions to be used with faster, which will increase coverage.
