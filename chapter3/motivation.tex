...
The problem of branch prediction accuracy can clearly be illustrated by one of the benchmarks explored in Chapter~\ref{chp:cases}, the SD-VBS program \bm{MSER}.
...
Thus, for a benchmark such as \bm{MSER}, the SF scheme leads to inefficiencies.
...
Section~\ref{} covers a new fetching scheme that aims to reduce the communication between cores by parallelising block fetches.
...
If multiple blocks representing Listing~\ref{lst:mser_snipet} are in flight, a block reading the value of \textit{nvisited} will have to wait for the previous block to execute the write.
...
Section~\ref{} covers the implementation of a value predictor for an EDGE processor which is used throughout this chapter.
