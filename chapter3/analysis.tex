...
This is a significant speedup, as Chapter~\ref{chp:cases} showed that \bm{Multi\_Ncut} was a difficult benchmark for core composition (1.3x speedup in Chapter~\ref{chp:cases}).
...
In fact, the spreading accross cores is the reason why some benchmarks perform worse: \bm{Disparity}, \bm{Texture\_Synthesis} and \bm{Tracking} see a slight performance decrease.
...
The performance of RRF is more apparent when taking value prediction into account.
...
\subsubsection{Round robin fetching scheme bottleneck Analysis}
...
The Figure shows that benchmarks \bm{Multi\_NCut} features a very low variance; this partially explains why RRF performs well even without value prediction.
