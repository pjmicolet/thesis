...
This section explores how a perfect branch and value predictor, paired with the new fetching scheme (RRF), improves the performance of core composition on the SD-VBS benchmarks.
...
The baseline for this section is normal fetching scheme (SF) with no value prediction (\novp) with perfect branch prediction.
...
\begin{figure}[t]
    ...
    \caption{Standard deviation of block sizes per benchmark. Blocks are grouped up in buckets (occupy 1, 2, 3 or 4 segments).}
    \label{fig:variance}
	\vspace{1em}
\end{figure}
