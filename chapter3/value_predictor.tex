...
In the EDGE architecture registers are primarily used to pass information between blocks, which can cause data dependencies.
...
%If cores are composed, and all have an iteration of the loop, they will have to wait on previous writes to be able to execute their loads; serialising execution of the loop.
%When register dependencies are a bottleneck and cannot be optimised via a compiler, core composition cannot be considered an effective method of improving performance.
...
%On the other hand the \textit{computational} predictor generates a prediction by adding a constant to the last committed value of the instruction.% stores the committed value of the instruction in a table called the Last Value Table (LVT), and stores the delta between the two previous values in another set of tables, this delta is called a \textit{stride}.
