...
The experiments done throughout this thesis use a cycle-accurate simulator for the EDGE architecture.
...
Throughout this thesis, a single core configuration was explored throughout.
Naturally, this means that the design-space exploration done throughout the thesis are specific to the configuration used.
Whilst the work conducted throughout the thesis provide a good methodology for generating models that automate reconfiguration, the overall improvements are mainly specific to the configuration.

\section{Future Work}

Compiler passes to improve performance of core composition.
For example, looking at modulo scheduling for loops that feature control flow to try and maximise block sizes.

Design space exploration on a micro-architectural level to determine what type of core is best for core composition.
For example, maybe having a processor with even smaller cores, therefore less power-hungry, may be best.

Exploring a runtime system that can do both speculative multithreading and core composition.
This may be useful for irregular parallelism where multithreading may be better when there's abundance of work, and core composition is better when there isn't.