...
A single experiment can take up to 6 hours on a single machine (ref), and the average Million instructions per second (MIPS) of the simulator is of 0.1 when only simulating a single core.
...
Instead a coarse grained power model is applied where power gating is applied either a core is in used and turned on or or it is off.