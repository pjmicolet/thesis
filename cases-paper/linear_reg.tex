Having shown the potential that a DMP has to offer, this section now presents a learning scheme that is used to exploit the large energy savings available.
...
As the decisions are made at runtime, a lightweight model that is able to predict the correct configuration that can be integreated in hardware is necesseray.
Therefore a linear regression which makes predictions using a weighted sum of the input feature is chosen.The model is trained offline using the traces gathered from the prior analysis for the \textbf{DSpeed} scenario which maximizes energy savings while maintaining performance.
Figure~\ref{fig:linmod} is a graphical representation of how the linear model is trained.
...
Therefore, when building the model, the linear regression considers that stores correlate with other features and selects to remove it as a variable.
