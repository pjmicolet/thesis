...
Dynamic core composition enables the ability to change the number of cores for each time tick (an interval of 640 blocks) during the execution of a program.
...
The whole application is run on 1,2,4,8 and 16 fused cores and the performance is recorded for each time tick.
...
SB does not always perform well neither; as can be seen, for the benchmarks \bm{Disparity}, \bm{Sift}, \bm{Texture\_Synthesis} the energy consumption is much higher.
