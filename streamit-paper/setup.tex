...
Figure~\ref{fig:overview} presents the workflow of the system used in this chapter and Figure~\ref{fig:examplestream} illustrates the workflow on a graph.
...
Table~\ref{tab:instancefilt} shows the number of filter instances for each of the benchmarks whilst Table~\ref{tab:splitjoin} shows the number of SplitJoins in each benchmark.
...
% The FFT need are variable
 ...
 \hline
  ...
  131 & 29 & 6 & 4 & 13 \\ \hline
 \end{tabular}
  \caption{Number of filter instances present in each benchmark.}\label{tab:instancefilt}
\end{table}
...
Whilst there exists at least 30 realistic applications for StreamIt~\cite{thiesStreamit2010} this is simply not enough to create a large enough data set.
